...
function getEventConfig(eventType: string, summary: string) {
  if (summary.startsWith("Forked")) {
    return { icon: GitFork, label: "forked", iconColor: "text-orange-600 dark:text-orange-400", bgColor: "bg-orange-50 dark:bg-orange-950/40" };
  }
  if (summary.startsWith("Starred")) {
    return { icon: Star, label: "starred", iconColor: "text-yellow-600 dark:text-yellow-400", bgColor: "bg-yellow-50 dark:bg-yellow-950/40" };
  }
  if (summary.startsWith("Created repository")) {
    return { icon: Plus, label: "created", iconColor: "text-teal-600 dark:text-teal-400", bgColor: "bg-teal-50 dark:bg-teal-950/40" };
  }
  if (
    summary.startsWith("Created branch") ||
    summary.startsWith("Created tag")
  ) {
    return { icon: GitBranch, label: "created", iconColor: "text-cyan-600 dark:text-cyan-400", bgColor: "bg-cyan-50 dark:bg-cyan-950/40" };
  }
  if (summary.startsWith("Issue")) {
    return { icon: CircleDot, label: "issue on", iconColor: "text-rose-600 dark:text-rose-400", bgColor: "bg-rose-50 dark:bg-rose-950/40" };
  }

  switch (eventType) {
    case "push":
      return { icon: GitCommit, label: "pushed to", iconColor: "text-emerald-600 dark:text-emerald-400", bgColor: "bg-emerald-50 dark:bg-emerald-950/40" };
    case "pull_request":
      return { icon: GitPullRequest, label: "PR on", iconColor: "text-purple-600 dark:text-purple-400", bgColor: "bg-purple-50 dark:bg-purple-950/40" };
    case "release":
      return { icon: Tag, label: "released", iconColor: "text-blue-600 dark:text-blue-400", bgColor: "bg-blue-50 dark:bg-blue-950/40" };
    default:
      return { icon: GitBranch, label: "updated", iconColor: "text-gray-600 dark:text-gray-400", bgColor: "bg-gray-50 dark:bg-neutral-800/60" };
  }
}
    ...
    <Card className="transition-shadow hover:shadow-geist-3">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className={`h-8 w-8 rounded-geist-sm ${config.bgColor} flex items-center justify-center shrink-0`}>
